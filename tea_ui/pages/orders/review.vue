		...
		<view class="content">
			<list-cell last class="mb-20" :hover="false">
				<view class="w-100 d-flex justify-content-between align-items-center">
					<view class="font-size-lg text-color-base flex-fill">订单号：{{ orderNo }}</view>
				</view>
			</list-cell>
	...
	export default {
		...
		methods: {
			submit() {
				...
				this.$request('/order/createOrderComment', 'post', {
					serviceNum: this.form.score_service,
					orderNum: this.form.score_order,
					speedNum: this.form.score_speed,
					productNum: this.form.score_product,
					enviromentNum: this.form.score_enviroment,
