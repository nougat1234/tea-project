						...
						<view class="d-flex align-items-center overflow-hidden" >
							...
							<view v-else>
								<view class="store-name">
									<text v-model="storeName">{{storeName}}</text>
								<!-- 	<view class="iconfont iconarrow-right" @tap="detail"></view> -->
								</view>
								<view class="store-location">
									<image src='/static/images/order/location.png' style="width: 30rpx; height: 30rpx;" class="mr-10"></image>
									<text>距离您 1.5 km</text>
								</view>
...
export default {
	...
	methods: {
		...
		async init() {
			...
			this.storeName = this.stores[index];
		...
		// 查看订单详情
		detail() {
			uni.navigateTo({ url: '/pages/menu/search' });
		}
	}
};
</script>
